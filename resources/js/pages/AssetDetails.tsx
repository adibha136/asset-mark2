...
const AssetDetails = () => {
  ...
  const exportToExcel = () => {
    ...
    const exportData = [{
      ...
      "Cost": asset.cost || "-",
      "Description": asset.description?.replace(/<[^>]*>/g, '') || "-" // Strip HTML tags
    }];

    const worksheet = XLSX.utils.json_to_sheet(exportData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Asset Detail");
    XLSX.writeFile(workbook, `Asset_Detail_${asset.name.replace(/\s+/g, '_')}_${format(new Date(), "yyyy-MM-dd")}.xlsx`);
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" className="btn-gradient" onClick={exportToExcel}>
