...
import {
  ...
  AlertCircle, RefreshCw, Zap, FileSpreadsheet
} from "lucide-react";
...
export default function ManageUserAssets() {
  ...
  const { data: userData, refetch: refetchUserAssets, isLoading: isLoadingUserData } = useQuery({
  ...
  const exportToExcel = () => {
    const exportData = assignedAssets.map((asset: any) => ({
      "User Name": user?.name || "-",
      "User Email": user?.email || "-",
      "User Phone": user?.phone || user?.mobile_phone || "-",
      ...
      "Expiry": (asset.type === 'hardware' ? asset.warranty_expiry : asset.license_expiry)
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "User Assets");
    XLSX.writeFile(workbook, `Assets_${user?.name || "User"}_${format(new Date(), "yyyy-MM-dd")}.xlsx`);
  };
