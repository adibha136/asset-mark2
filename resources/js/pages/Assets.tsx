...
export default function Assets() {
  ...
  const derivedStats = useMemo(() => {
    ...
    return {
      ...
      total_users: baseFilteredUsers.length,
    };
  ...
  const displayStats = useMemo(() => {
    const isFiltered = searchQuery !== "" || domainFilter !== "all";
    if (!isFiltered && stats) {
      return stats;
    }
    return derivedStats;
  }, [stats, derivedStats, searchQuery, domainFilter]);
    ...
    if (viewMode === 'assets') {
      mainData = filteredAssets.map(asset => {
        ...
        return {
          ...
          "Assigned To Phone": assignedUsers.map(u => u.phone || u.mobile_phone || "-").join(", ") || "-",
          "Assigned To License": assignedUsers.map(u => u.license_name || "No License").join(", ") || "-",
          "Expiry/Warranty": (asset.type === 'hardware' ? asset.warranty_expiry : asset.license_expiry) 
            ? format(new Date((asset.type === 'hardware' ? asset.warranty_expiry : asset.license_expiry) as string), "yyyy-MM-dd") 
            : "-",
  ...
  const userColumns = [
    ...
    {
      key: "license",
      header: "M365 Licenses",
      render: (user: DirectoryUser) => {
        const count = user.license_name && user.license_name !== 'No License' 
          ? user.license_name.split(',').length 
          : 0;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button
            ...
            disabled={isLoadingAssets || isLoadingUsers}
            className="h-9 gap-2"
          >
      ...
      <div className="flex flex-col md:flex-row gap-3">
        ...
        <div className="flex flex-wrap gap-4 w-full">
          <div
            className={cn(
              "bg-card border rounded-xl p-4 flex items-center gap-4 cursor-pointer hover:border-blue-500/50 transition-colors min-w-[200px] flex-1",
              viewMode === 'users' && statusFilter === 'all' && "border-blue-500 bg-blue-500/5"
            )}
            onClick={() => { setStatusFilter('all'); setViewMode('users'); }}
          >
            ...
            <div>
              ...
              <p className="text-lg font-bold">{displayStats.total_users}</p>
            </div>
          </div>

          {displayStats.used_licenses > 0 && (
            <div
              className={cn(
            ...
            >
              ...
              <div>
                ...
                <p className="text-lg font-bold">{displayStats.used_licenses}</p>
              </div>
            </div>
          )}

          {displayStats.no_license_count > 0 && (
            <div
              className={cn(
            ...
            >
              ...
              <div>
                ...
                <p className="text-lg font-bold">{displayStats.no_license_count}</p>
              </div>
            </div>
          )}
          ...
          >
            ...
            <div>
              ...
              <p className="text-lg font-bold">{displayStats.asset_count}</p>
            </div>
          </div>

          {displayStats.active_users > 0 && (
            <div
              className={cn(
            ...
            >
              ...
              <div>
                ...
                <p className="text-lg font-bold">{displayStats.active_users}</p>
              </div>
            </div>
          )}

          {displayStats.inactive_users > 0 && (
            <div
              className={cn(
            ...
            >
              ...
              <div>
                ...
                <p className="text-lg font-bold">{displayStats.inactive_users}</p>
              </div>
            </div>
          )}
        </div>
